...
const dev = {
  run() {
    Promise.all([this.devRenderer(), mainBuilder()])
      .then(() => {
        this.startElectron();
      })
      .catch((err) => {
  ...
  devRenderer() {
    return new Promise((resolve, reject) => {
      console.log("Boost render process debugging......");
  ...
  electronLog(data, color) {
    ...
    data
      ...
      .forEach((line) => (log += `\n${line}`));
    if (/[0-9A-z]+/.test(log)) {
      console.log(chalk[color].bold("Electron -------------------") + log + chalk[color].bold("----------------------------"));
    }
  },
  // 重新打包，只打包主进程
  reBuildApp() {
    mainBuilder("development")
      .then(() => {
        this.startElectron();
      })
      .catch((err) => {
